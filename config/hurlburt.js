...
module.exports = {
  ...
  messages: {
    ...
    corrections: {
      tooGeneric: '🔍 Попробуй описать более конкретно. Что ИМЕННО происходило?',
      theoretical: '💡 Опиши не то, что ты думаешь, а что БЫЛО в тот момент',
      retrospective: '⏰ Вернись к МОМЕНТУ сигнала. Что было именно тогда?',
      future: '🎯 Фокус на моменте! Не на планах, а на том, что БЫЛО',
      tooLong: '✂️ Слишком много деталей. Что было ГЛАВНЫМ в тот момент?',
      abstract: '🌱 Слишком абстрактно. Опиши конкретный опыт'
    }
  },
  ...
  // Система эскалации уведомлений
  escalation: {
    // Время ожидания ответа до начала эскалации (в минутах)
    responseTimeoutMinutes: 60,
    
    // Максимальный уровень эскалации
    maxEscalationLevel: 10,
    
    // Интервалы между уведомлениями по уровням (в минутах)
    intervals: {
      level1: { min: 10, max: 30 },    // Первый уровень: 10-30 мин
      level2: { min: 8, max: 25 },     // Второй уровень: 8-25 мин
      level3: { min: 5, max: 20 },     // Третий уровень: 5-20 мин
      level4: { min: 5, max: 15 },     // Четвертый уровень: 5-15 мин
      level5: { min: 3, max: 12 },     // Пятый уровень: 3-12 мин
      level6: { min: 3, max: 10 },     // Шестой уровень: 3-10 мин
      level7: { min: 2, max: 8 },      // Седьмой уровень: 2-8 мин
      level8: { min: 2, max: 6 },      // Восьмой уровень: 2-6 мин
      level9: { min: 1, max: 5 },      // Девятый уровень: 1-5 мин
      level10: { min: 1, max: 3 }      // Максимальный уровень: 1-3 мин
    },
    
    // Сообщения для разных уровней эскалации
    messages: {
      level1: '🔔 Время для короткого опроса!',
      level2: '⏰ Не забудьте про опрос!',
      level3: '📱 Быстрый опрос ждет вас!',
      level4: '🎯 Всего 2-3 минуты для опроса!',
      level5: '💡 Ваши данные важны - пройдите опрос!',
      level6: '🔴 Напоминание: пожалуйста, ответьте на опрос',
      level7: '⚡ Срочно: завершите опрос для исследования',
      level8: '🚨 Важно: ваш ответ нужен для науки!',
      level9: '🔥 Последний шанс ответить на опрос!',
      level10: '🆘 Критически важно: завершите опрос!'
    },
    
    // Когда прекратить эскалацию
    stopConditions: {
      // Максимальное время эскалации (в часах)
      maxEscalationHours: 4,
      
      // Прекратить если пользователь вне временного окна
      respectTimeWindow: true,
      
      // Прекратить если достигли дневного лимита уведомлений
